<?php

if ( ! defined( 'ABSPATH' ) ) exit;

if ( ! class_exists( 'Taiowc_Main' ) ):

    class Taiowc_Main {
        /**
         * Member Variable
         *
         * @var object instance
         */
       private static $instance;
       private $taiowc_set_api;
       public  $taiowc_cartInstances = 0 ;
       public  $taiowc_notices = array();
       private $isSideCartPage;

       /**
         * Initiator
        */

        public static function instance() {

            if ( ! isset( self::$instance ) ) {

                self::$instance = new self();

            }

            return self::$instance;

        }

        /**
         * Constructor
         */
        public function __construct(){

        

        $this->taiowc_includes();

        $this->taiowc_hooks();
        
        }

        public function taiowc_includes() {

                require_once TAIOWC_PLUGIN_PATH . '/inc/taiowc-markup.php';
                require_once TAIOWC_PLUGIN_PATH . '/inc/taiowc-cart-fragment.php';
                require_once TAIOWC_PLUGIN_PATH . '/inc/taiowc-style.php';
                
        }

        public function taiowc_hooks() {

                add_action( 'init', array( $this, 'taiowc_settings_api' ), 5 );

                if($this->taiowc_is_wc_active()){ 

                add_filter( 'body_class', array( $this, 'taiowc_body_class' ) );

                add_filter( 'woocommerce_post_class',array( $this, 'taiowc_woo_post_class' )  ); 

                add_shortcode( 'taiowc', array( $this, 'taiowc_addBody' ), 5 );

                add_action( 'wp_enqueue_scripts', array( $this, 'taiowc_scripts' ), 1 );

                add_action('taiowc_cart_show_icon',array( $this,'taiowc_cart_icon'),1,1);

                add_action( 'wp_footer', array( $this, 'taiowc_addcartBody' ) );

                add_action( 'taiowc_mini_cart', array( $this, 'taiowc_mini_cart_content' ) );

                add_action( 'taiowc_mini_cart_empty', array( $this, 'taiowc_mini_cart_empty_content' ) );
                
                add_action( 'wp_ajax_taiowc_create_nonces', array( $this,'taiowc_create_nonces'));
                add_action( 'wp_ajax_nopriv_taiowc_create_nonces', array( $this,'taiowc_create_nonces'));

                add_action( 'wc_ajax_taiowc_update_item_quantity', array( $this,'taiowc_update_item_quantity'));

                add_action( 'wc_ajax_taiowc_add_item_cart', array( $this,'taiowc_add_item_cart'));

                add_action( 'wc_ajax_taiowc_undo_item', array( $this,'taiowc_undo_item'));
                
            }

        }

        
        
        public function taiowc_is_wc_active() {

            return class_exists( 'WooCommerce' );

        }
        
       
    
        public function taiowc_is_required_php_version() {

            return version_compare( PHP_VERSION, '5.6.0', '>=' );

        }

        public function taiowc_settings_api() {

            if ( ! $this->taiowc_set_api ){

                $this->taiowc_set_api = new taiowc_Set();

            }

            return $this->taiowc_set_api;
        }

        public function taiowc_get_option( $id ) {
           
            if ( ! $this->taiowc_set_api ) {

                $this->taiowc_settings_api();

            }
            
            return $this->taiowc_set_api->taiowc_get_option( $id );
        }

        public function taiowc_get_options() {

            return get_option( 'taiowc' );
            
        }

        public function taiowc_body_class( $classes ) {
           
            $old_classes = $classes;

            if ( apply_filters( 'taiowc_body_class_disable', false ) ) {

                return $classes;

            }

            array_push( $classes, 'taiowc' );

            if ( wp_is_mobile() ) {

                array_push( $classes, 'taiowc-on-mobile' );

            }
            
            return apply_filters(  'taiowc_body_class_', array_unique( $classes ), $old_classes );
        }

        public function taiowc_woo_post_class( $classes ) {

            $old_classes = $classes;

            if(taiowc_main()->taiowc_get_option('taiowc-cart_open')=='fly-image-open'){

            array_push( $classes, 'taiowc-fly-cart' );

            } 

            return apply_filters(  'taiowc_woo_post_class_', array_unique( $classes ), $old_classes );
        }

        public function taiowc_scripts(){

              wp_enqueue_style( 'taiowc-style', TAIOWC_PLUGIN_URI. '/assets/css/taiowc-style.css', array(), '1.1.3' );

              wp_enqueue_style( 'taiowc-owl.carousel-style', TAIOWC_PLUGIN_URI. '/assets/css/owl.carousel.css', array(), TAIOWC_VERSION );

              wp_enqueue_style( 'th-icon-css', TAIOWC_PLUGIN_URI. '/th-icon/style.css', array(), TAIOWC_VERSION );
              
              wp_enqueue_style('dashicons');
              
              wp_add_inline_style('taiowc-style', taiowc_style_pro());

              wp_enqueue_script( 'taiowc-cart-script', TAIOWC_PLUGIN_URI. '/assets/js/taiowc-cart.js', array( 'jquery' ), TAIOWC_VERSION, true);

              wp_enqueue_script( 'taiowc-owl.carousel-script', TAIOWC_PLUGIN_URI. '/assets/js/owl.carousel.js', array( 'jquery' ),true);
               
              if(taiowc_main()->taiowc_get_option('taiowc-cart_open')=='fly-image-open'){

               wp_enqueue_script( 'taiowc-fly-cart-script', TAIOWC_PLUGIN_URI. '/assets/js/taiowc-flycart.js', array( 'jquery' ),true);

               wp_enqueue_script('taiowc-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js');

              }
                
              wp_enqueue_script( 'wc-cart-fragments' );
             
              $noticeMarkup = '<ul class="taiowc-notices-msg">%s</ul>'; 

              wp_localize_script(

                'taiowc-cart-script', 'taiowc_param', array(

                    'ajax_url'             => esc_url(admin_url( 'admin-ajax.php' )),

                    'wc_ajax_url'          => esc_url(WC_Ajax::get_endpoint( "%%endpoint%%" )),

                    'update_qty_nonce'     => wp_create_nonce( 'taiowc_update_qty_nonce' ),

                    'apply_coupon_nonce'   => wp_create_nonce('apply-coupon'),

                    'remove_coupon_nonce'  => wp_create_nonce('remove-coupon'),

                    'update_shipping_method_nonce' => wp_create_nonce( 'update-shipping-method' ),    

                    'taiowc_cart_open' => esc_html(taiowc_main()->taiowc_get_option('taiowc-cart_open')),
                    'cart_effect'       => esc_html(taiowc_main()->taiowc_get_option('taiowc-cart_effect')),
                    
                )
            );
        }

        public function taiowc_add_setting( $tab_id, $tab_title, $tab_sections, $active = false, $is_pro_tab = false, $is_new = false ) {
            add_filter(
                'taiowc_settings', function ( $fields ) use ( $tab_id, $tab_title, $tab_sections, $active, $is_pro_tab, $is_new ) {
                array_push(
                    $fields, array(
                        'id'       => $tab_id,
                        'title'    => esc_html( $tab_title ),
                        'active'   => $active,
                        'sections' => $tab_sections,
                        'is_pro'   => $is_pro_tab,
                        'is_new'   => $is_new
                    )
                );

                return $fields;
            }
          );
        }
       
      /*****************/
      // ADD SHORTCODE
      /*****************/
       public function taiowc_addBody( $atts, $content, $tag ) {

        $crtArgs = shortcode_atts( array(
            'layout'         => '',
            'iconstyle'      => '',
        ), $atts, $tag );

        $args = apply_filters( 'taiowc_shortcode_arg', $crtArgs );

        return self::taiowc_getCart($args );

       }

       public function taiowc_getCart( $args ) {

        if( !$this->taiowc_HideCartPage() ) return;
       
        ob_start();

        $filename = apply_filters( 'taiowc_path', TAIOWC_PLUGIN_PATH . '/inc/taiowc-cart.php' );

        if ( file_exists( $filename ) ) {

            include $filename;

            if ( function_exists( 'opcache_invalidate' ) ) {

                @opcache_invalidate( $filename, true );

            }

        }

        $html = ob_get_clean();

        return apply_filters( 'taiowc_html', $html, $args );

       }

       public function taiowc_cart_icon($iconstyle){
        
              if($iconstyle){

                if($iconstyle=='icon-1'){ ?>
                 
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shopping-bag w-6 h-6 stroke-[1.5px]" aria-hidden="true"><path d="M16 10a4 4 0 0 1-8 0"></path><path d="M3.103 6.034h17.794"></path><path d="M3.4 5.467a2 2 0 0 0-.4 1.2V20a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6.667a2 2 0 0 0-.4-1.2l-2-2.667A2 2 0 0 0 17 2H7a2 2 0 0 0-1.6.8z"></path></svg>
      
                    <?php }elseif($iconstyle=='icon-2'){?>
      
                    <span class="th-icon th-icon-Shopping_icons-11"></span>
                     
                    <?php }elseif($iconstyle=='icon-3'){?>
      
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.153 4L6.01 15.146a.993.993 0 0 0 .327.603.997.997 0 0 0 .679.251H18a1 1 0 0 0 .949-.684l3-9A1 1 0 0 0 21 5H6.347L5.99 2.85a.993.993 0 0 0-.357-.625A.998.998 0 0 0 4.984 2H3a1 1 0 0 0 0 2h1.153zm3.694 10L6.68 7h12.933l-2.334 7H7.847zM10 20a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm9 0a2 2 0 1 1-4 0 2 2 0 0 1 4 0z" fill="currentColor"></path></svg>
      
                    <?php }elseif($iconstyle=='icon-4'){?>
      
                   <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 6H20V22H4V6Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"></path><path d="M9 9V6C9 4.34315 10.3431 3 12 3C13.6569 3 15 4.34315 15 6V9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path></svg>
      
                    <?php }elseif($iconstyle=='icon-5'){?>
      
                   <span class="th-icon th-icon-Shopping-Bag-Cart-Icons-11"></span>
      
                   <?php }elseif($iconstyle=='icon-6'){ ?>
      
                   <svg fill="currentColor" width="24" height="24" viewBox="0 0 24 24" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"><path d="M8.5,19A1.5,1.5,0,1,0,10,20.5,1.5,1.5,0,0,0,8.5,19ZM19,16H7a1,1,0,0,1,0-2h8.49121A3.0132,3.0132,0,0,0,18.376,11.82422L19.96143,6.2749A1.00009,1.00009,0,0,0,19,5H6.73907A3.00666,3.00666,0,0,0,3.92139,3H3A1,1,0,0,0,3,5h.92139a1.00459,1.00459,0,0,1,.96142.7251l.15552.54474.00024.00506L6.6792,12.01709A3.00006,3.00006,0,0,0,7,18H19a1,1,0,0,0,0-2ZM17.67432,7l-1.2212,4.27441A1.00458,1.00458,0,0,1,15.49121,12H8.75439l-.25494-.89221L7.32642,7ZM16.5,19A1.5,1.5,0,1,0,18,20.5,1.5,1.5,0,0,0,16.5,19Z"></path></svg>
                      
                   <?php }elseif($iconstyle=='icon-7'){?>
      
                  <span class="th-icon th-icon-Shopping_icons-01"></span>
      
                   <?php }

              }else{
                
              $icon_svg = taiowc_main()->taiowc_get_option( 'taiowc-cart-icon' );
              
              $icon_img_url = taiowc_main()->taiowc_get_option( 'taiowc-icon_url' );
               
              if( $icon_svg=='icon-1' ){ ?>
                 
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shopping-bag w-6 h-6 stroke-[1.5px]" aria-hidden="true"><path d="M16 10a4 4 0 0 1-8 0"></path><path d="M3.103 6.034h17.794"></path><path d="M3.4 5.467a2 2 0 0 0-.4 1.2V20a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6.667a2 2 0 0 0-.4-1.2l-2-2.667A2 2 0 0 0 17 2H7a2 2 0 0 0-1.6.8z"></path></svg>

              <?php }elseif($icon_svg=='icon-2'){?>

              <span class="th-icon th-icon-Shopping_icons-11"></span>
               
              <?php }elseif($icon_svg=='icon-3'){?>

             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.153 4L6.01 15.146a.993.993 0 0 0 .327.603.997.997 0 0 0 .679.251H18a1 1 0 0 0 .949-.684l3-9A1 1 0 0 0 21 5H6.347L5.99 2.85a.993.993 0 0 0-.357-.625A.998.998 0 0 0 4.984 2H3a1 1 0 0 0 0 2h1.153zm3.694 10L6.68 7h12.933l-2.334 7H7.847zM10 20a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm9 0a2 2 0 1 1-4 0 2 2 0 0 1 4 0z" fill="currentColor"></path></svg>

              <?php }elseif($icon_svg=='icon-4'){?>

             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 6H20V22H4V6Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"></path><path d="M9 9V6C9 4.34315 10.3431 3 12 3C13.6569 3 15 4.34315 15 6V9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path></svg>

              <?php }elseif($icon_svg=='icon-5'){?>

             <span class="th-icon th-icon-Shopping-Bag-Cart-Icons-11"></span>

             <?php }elseif($icon_svg=='icon-6'){ ?>

             <svg fill="currentColor" width="24" height="24" viewBox="0 0 24 24" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"><path d="M8.5,19A1.5,1.5,0,1,0,10,20.5,1.5,1.5,0,0,0,8.5,19ZM19,16H7a1,1,0,0,1,0-2h8.49121A3.0132,3.0132,0,0,0,18.376,11.82422L19.96143,6.2749A1.00009,1.00009,0,0,0,19,5H6.73907A3.00666,3.00666,0,0,0,3.92139,3H3A1,1,0,0,0,3,5h.92139a1.00459,1.00459,0,0,1,.96142.7251l.15552.54474.00024.00506L6.6792,12.01709A3.00006,3.00006,0,0,0,7,18H19a1,1,0,0,0,0-2ZM17.67432,7l-1.2212,4.27441A1.00458,1.00458,0,0,1,15.49121,12H8.75439l-.25494-.89221L7.32642,7ZM16.5,19A1.5,1.5,0,1,0,18,20.5,1.5,1.5,0,0,0,16.5,19Z"></path></svg>
                
             <?php }elseif($icon_svg=='icon-8'){ ?>

             <span class="th-icon th-icon-Shopping_icons-01"></span>

                
             <?php }elseif($icon_svg=='icon-7'){

                 if($icon_img_url!==''){

                    ?>

                <img src="<?php echo esc_url($icon_img_url); ?>" class="taiowc-cart-icon-img">


             <?php } 

             }

            }

         }

         public function taiowc_show_cart_in_pages(){

            if(!class_exists( 'WooCommerce' )){
             return;
            }
        
            $cart_page     = taiowc_main()->taiowc_get_option( 'taiowc-hide-cartp' );
            $checkout_page = taiowc_main()->taiowc_get_option( 'taiowc-hide-checkoutp' );
            $shop_page     = taiowc_main()->taiowc_get_option( 'taiowc-hide-shopp' );
            $account_page  = taiowc_main()->taiowc_get_option( 'taiowc-hide-accountp' );
            $single_page   = taiowc_main()->taiowc_get_option( 'taiowc-hide-singlep' );
            $home_page     = taiowc_main()->taiowc_get_option( 'taiowc-hide-homep' );
            $blog_page     = taiowc_main()->taiowc_get_option( 'taiowc-hide-blogp' );
        
            $hidepage = false;
        
            if(is_cart() && $cart_page == true){
                $hidepage = true;
            }elseif(is_checkout() && $checkout_page == true){
                $hidepage = true;
            }elseif((is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag()) && ($shop_page == true)){
                $hidepage = true;
            }elseif(is_account_page() && $account_page == true){
                $hidepage = true;
            }elseif(is_product() && $single_page == true){
                $hidepage = true;
            }elseif((is_home() || is_front_page()) && $home_page == true){
                $hidepage = true;
            }elseif((is_archive() || is_author() || is_category() || (is_home() && !is_front_page()) || is_single() || is_tag()) && $blog_page == true){
                $hidepage = true;
            }else{
                $hidepage = false;
            }
        
            return $hidepage;
        
          }
         public function taiowc_addcartBody(){

            if( $this->taiowc_show_cart_in_pages() == true ){
                return;
            }
            
            if(taiowc_main()->taiowc_get_option( 'taiowc-show_cart' ) == true){
       
                if(taiowc_main()->taiowc_get_option( 'taiowc-cart_style' ) == 'style-1'){

                     echo do_shortcode('[taiowc layout="cart_fixed_1"]');

                }elseif(taiowc_main()->taiowc_get_option( 'taiowc-cart_style' ) == 'style-2'){

                     echo do_shortcode('[taiowc layout="cart_fixed_2"]');

                }

             }

         }


        public function taiowc_get_cart_count(){

                if( taiowc_main()->taiowc_get_option( 'taiowc-basket_count' ) == 'numb_prd' ){

                    return count( WC()->cart->get_cart() );
                }
                else{

                    return WC()->cart->get_cart_contents_count();
                }

            }

        public function taiowc_mini_cart_content(){ ?>

        <?php if ( WC()->cart && ! WC()->cart->is_empty() ) : ?>     

        <div class="woocommerce-mini-cart cart_list taiowc-mini-cart-list">

        <?php

        if( taiowc_main()->taiowc_get_option( 'taiowc-cart_item_order' ) == 'prd_first' ){

                 $get_cart_content =  array_reverse( WC()->cart->get_cart() );

            }else{

                $get_cart_content = WC()->cart->get_cart();

        }
       

        foreach ( $get_cart_content as $cart_item_key => $cart_item ) {

            $_product   = apply_filters( 'woocommerce_cart_item_product', $cart_item['data'], $cart_item, $cart_item_key );

            $product_id = apply_filters( 'woocommerce_cart_item_product_id', $cart_item['product_id'], $cart_item, $cart_item_key );

            if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {

                $product_name      = apply_filters( 'woocommerce_cart_item_name', $_product->get_name(), $cart_item, $cart_item_key );

                $thumbnail         = apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image(), $cart_item, $cart_item_key );

                $product_price     = apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );

                $product_permalink = apply_filters( 'woocommerce_cart_item_permalink', $_product->is_visible() ? $_product->get_permalink( $cart_item ) : '', $cart_item, $cart_item_key );

                $rating_count   =  $_product->get_rating_count();

                $average        =  $_product->get_average_rating();

                $rating         = apply_filters( 'woocommerce_cart_item_rating', wc_get_rating_html( $average, $rating_count ), $cart_item, $cart_item_key );

                $quantity_text = esc_html__('Quantity','th-all-in-one-woo-cart');

                $quant = $this->taiowc_mini_cart_add_quantity($_product,$cart_item_key,$cart_item);

                if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_img' ) == false){

                $thumbnail ='';

                }

                if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_title' ) == false){
                    
                $product_name ='';

                }

                if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_price' ) == false){
                    
                $product_price ='';

                }
               
                if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_quantity' ) == false){ 

                $quant = '';
                $quantity_text = '';

                }

                ?>
                <div class="taiowc-woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
                    <div class="item-image-wrapper">
                        <?php echo wp_kses_post($thumbnail); ?>
                    </div>
                    <div class="item-product-wrap">

                    <?php
                    echo apply_filters( 
                        'woocommerce_cart_item_remove_link',
                        sprintf(
                            '<a class="taiowc-remove-item taiowc_remove_from_cart_button" aria-label="%s" data-product_id="%s" data-key="%s" data-product_sku="%s"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x" aria-hidden="true"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg> </a>',
                            esc_attr__( 'Remove this item', 'th-all-in-one-woo-cart' ),
                            esc_attr( $product_id ),
                            esc_attr( $cart_item_key ),
                            esc_attr( $_product->get_sku() )
                        ),
                        $cart_item_key
                    );
                    
                    ?>

                    <?php

                     if ( empty( $product_permalink ) ) : ?>

                     <?php 

                        
                        echo '<h4>';
                        echo wp_kses_post($product_name); 
                        echo '</h4>';

                        if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_rating' ) == true){

                        echo wp_kses_post(apply_filters( 'woocommerce_cart_item_rating', wc_get_rating_html( $average, $rating_count ), $cart_item, $cart_item_key ));

                        }

                        echo wp_kses_post(wc_get_formatted_cart_item_data( $cart_item )); 
                        ?>

                    <?php else : ?>
                        <h4>
                        <a href="<?php echo esc_url( $product_permalink ); ?>">

                        <?php 


                        echo wp_kses_post($product_name);

                        if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_rating' ) == true){

                        echo wp_kses_post(apply_filters( 'woocommerce_cart_item_rating', wc_get_rating_html( $average, $rating_count ), $cart_item, $cart_item_key ));
                         
                        }
                       
                        ?>

                        </a></h4>

                         <?php echo wp_kses_post(wc_get_formatted_cart_item_data( $cart_item )); ?>

                    <?php endif; 

                     ?>

                     <?php if(taiowc_main()->taiowc_get_option( 'taiowc-show_prd_quantity' ) || taiowc_main()->taiowc_get_option( 'taiowc-show_prd_price' ) == true){ ?>

                  <div class="item-product-quantity">
                    <?php 
                    // Allowable HTML tags for security
                    $taiowc_allow_tag = array( 
                        'input' => array( 
                            'id' => array(),
                            'class' => array(),
                            'name' => array(),
                            'value' => array(),
                            'step' => array(),
                            'max' => array(),
                            'min' => array(),
                            'data-key' => array(),
                            'title' => array(),
                            'size' => array(),
                            'type' => array(),
                        ),
                        'button' => array(
                            'class' => array(),
                            'data-key' => array(),
                            'type' => array(),
                            'aria-label' => array(),
                        ),
                    );

                    // Add + and - buttons around the quantity input field
                    $quantity_html = sprintf(
                        '<button type="button" class="quantity-decrement" data-key="%1$s" aria-label="Decrease quantity">－</button>
                        %2$s
                        <button type="button" class="quantity-increment" data-key="%1$s" aria-label="Increase quantity">＋</button>',
                        esc_attr($cart_item_key),
                        wp_kses($quant, $taiowc_allow_tag)
                    );

                    // Display the quantity with custom buttons
                    echo apply_filters(
                        'woocommerce_widget_cart_item_quantity',
                        sprintf('<span class="quantity"><span class="quantity-text">%1$s</span><div class="quantity-wrap">%2$s</div> %3$s</span>',
                            esc_html($quantity_text),
                            $quantity_html,
                            wp_kses_post($product_price)
                        ),
                        $cart_item,
                        $cart_item_key
                    );
                    ?>
                </div>

           <?php } ?>

                </div>

                
          

            </div>

                <?php

            }
        }
        
        ?>
    </div>
     
    <?php else : 

    do_action('taiowc_mini_cart_empty');

    endif;

  }    
 
    public function taiowc_mini_cart_empty_content(){ 

             if(taiowc_main()->taiowc_get_option( 'taiowc-empty_cart_url' )){
              
              $empty_btn_url = taiowc_main()->taiowc_get_option( 'taiowc-empty_cart_url' );

             }else{

              $empty_btn_url = get_permalink( wc_get_page_id( 'shop' ) );

             }

        ?>
            <div class="back-toshop-wrap">
                <div class="svg-wrapper">
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shopping-bag text-gray-300" aria-hidden="true"><path d="M16 10a4 4 0 0 1-8 0"></path><path d="M3.103 6.034h17.794"></path><path d="M3.4 5.467a2 2 0 0 0-.4 1.2V20a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6.667a2 2 0 0 0-.4-1.2l-2-2.667A2 2 0 0 0 17 2H7a2 2 0 0 0-1.6.8z"></path></svg></div>
             <p class="woocommerce-mini-cart__empty-message"><?php esc_html_e( 'Your Cart is Empty', 'th-all-in-one-woo-cart' ); ?></p>

             <a href="<?php echo esc_url($empty_btn_url);?>" class="woocommerce-back-to-shop"><?php echo esc_html(taiowc_main()->taiowc_get_option( 'taiowc-empty_cart_txt' )); ?></a>
            </div>

            <a class="taiowc-continue-shop"><?php esc_html_e('Continue Shopping','th-all-in-one-woo-cart'); ?></a>

    <?php }

    public function taiowc_mini_cart_add_quantity($_product,$cart_item_key,$cart_item){ 
                
                if ( $_product->is_sold_individually() ) {

                    $product_quantity = sprintf( '1 <input type="hidden" name="cart[%s][qty]" value="1" />', $cart_item_key );

                     } else {

                     $min = 0;

                     $max = $_product->get_max_purchase_quantity();

                    // Apply sanity to min/max args - min cannot be lower than 0.
                    $min = max( $min, 0 );

                    $max = 0 < $max ? $max : '';

                    // Max cannot be lower than min if defined.
                    if ( '' !== $max && $max < $min ) {

                      $max = $min;

                    }

                                    $input_id     = uniqid( 'quantity_' );
                                    $input_name   = "cart[{$cart_item_key}][qty]";
                                    $classes      = "taiowc-quantity input-text qty text";
                                    $input_value  = $cart_item['quantity'];
                                    $max_value    = $max;
                                    $min_value    = $min;
                                    $product_id   = $cart_item_key;
                                    

                            $product_quantity = sprintf( '<input type="number" id="%s" class="%s" name="%s" value="%s" step="1" max="%s" min="%s" data-key="%s"  title="Qty" size="4" >',esc_attr($input_id), esc_attr($classes), esc_attr($input_name), esc_attr($input_value), esc_attr($max_value), esc_attr($min_value) , esc_attr($product_id));

                        }

                        return $product_quantity;

     }    


    public function taiowc_create_nonces(){

        $actions = array(
            'apply-coupon',
            'remove-coupon',
            'update-shipping-method'
        );

        $nonces = array();

        foreach ($actions as $action) {
            $nonces[$action] = wp_create_nonce( $action );
        }

        wp_send_json( $nonces );
    }

    //add item cart

        public function taiowc_add_item_cart(){

        check_ajax_referer( 'taiowc_update_qty_nonce', 'security' );

        $product_id = isset($_POST['product_id']) ? absint( wp_unslash($_POST['product_id']) ) : 0;

        $new_qty = isset( $_POST['new_qty'] ) ? (int)( wp_unslash($_POST['new_qty'] )) : 0;
        

            // ✅ Add to cart with quantity
            $added = WC()->cart->add_to_cart( $product_id);

         if( $added ){

                $notice = esc_html__( 'Product Added', 'th-all-in-one-woo-cart' );

                $this->taiowc_set_notice( $notice, 'success' );
           }


        WC_AJAX::get_refreshed_fragments();

        die();

      }

     // update quantity

        public function taiowc_update_item_quantity(){

        check_ajax_referer( 'taiowc_update_qty_nonce', 'security' );

    $cart_key   = isset($_POST['cart_key']) ? sanitize_key(wp_unslash($_POST['cart_key'])) : '';

        $new_qty = isset( $_POST['new_qty'] ) ? floatval( wp_unslash($_POST['new_qty'] )) : 0;

        if( !is_numeric( $new_qty ) || $new_qty < 0 || !$cart_key ){

        $this->taiowc_set_notice( esc_html__( 'Something went wrong', 'th-all-in-one-woo-cart' ) );

        }
        
        $validated = apply_filters( 'taiowc_update_quantity', true, $cart_key, $new_qty );

        if( $validated && !empty( WC()->cart->get_cart_item( $cart_key ) ) ){

            $updated = $new_qty == 0 ? WC()->cart->remove_cart_item( $cart_key ) : WC()->cart->set_quantity( $cart_key, $new_qty );

            if( $updated ){

                if( $new_qty == 0 ){

                    $notice = esc_html__( 'Product removed', 'th-all-in-one-woo-cart' );

                    $notice .= ' <span class="taiowc-undo-item" data-key="'.$cart_key.'">'.esc_html__('Undo?','th-all-in-one-woo-cart').'</span>';  

                }
                else{

                    $notice = esc_html__( 'Cart Updated', 'th-all-in-one-woo-cart' );
                }

                $this->taiowc_set_notice( $notice, 'success' );
                
            }
        }

        WC_AJAX::get_refreshed_fragments();

        die();

      }

      //undo item

        public function taiowc_undo_item() {

            // ✅ Nonce verification (MOST IMPORTANT)
            check_ajax_referer( 'taiowc_undo_nonce', 'security' );

            // ✅ Validate + sanitize input properly
            $cart_key = isset($_POST['cart_key']) 
                ? sanitize_key( wp_unslash($_POST['cart_key']) ) 
                : '';

            if ( empty($cart_key) ) {
                wp_die();
            }

            $cart_success = WC()->cart->restore_cart_item( $cart_key );

            if ( $cart_success ) {
                $notice = esc_html__( 'Product restore', 'th-all-in-one-woo-cart' );
                $this->taiowc_set_notice( $notice, 'success' );
            }

            WC_AJAX::get_refreshed_fragments();

            wp_die(); // ✅ WordPress standard
}

        public function taiowc_set_notice( $notice, $type = 'success' ){

        $this->taiowc_notices[] = $this->taiowc_notice_html( $notice, $type );

       }


       public function taiowc_notice_html( $message, $notice_type = 'success' ){

        $taiowc_allowed_tag = array(

                                'span' => array(
                                   
                                    'data-key' => array(),
                                    'class'=> array(),
                                    
                                ),
                                
                            );
        $classes = $notice_type === 'error' ? 'taiowc-notice-error' : 'taiowc-notice-success';
        
        $html = '<li class="'.esc_attr($classes).'">'.wp_kses($message,$taiowc_allowed_tag).'</li>';
        
        return apply_filters( 'taiowc_notice_html', $html, $message, $notice_type );

     }

     public function taiowc_print_notices_html( $section = 'cart', $wc_cart_notices = true ){
        if (!WC()->cart){
            return;
        }
        if (!function_exists("wc_get_notices")) return;
          
        if( isset( $_POST['noticeSection'] ) && $_POST['noticeSection'] !== $section ) return;
        
        if( $wc_cart_notices ){

            do_action( 'woocommerce_check_cart_items' );

            $wc_notices = wc_get_notices( 'error' );

            foreach ( $wc_notices as $wc_notice ) {
                $this->taiowc_set_notice( $wc_notice['notice'], 'error' );
            }

            wc_clear_notices();

        }

        $taiowc_notices = apply_filters('taiowc_notices_before_print', $this->taiowc_notices, $section );

        $notices_html = sprintf('<div class="taiowc-notice-container" data-section="%1$s"><ul class="taiowc-notices">%2$s</ul></div>', $section, implode( '' , $taiowc_notices )  );

        echo wp_kses_post(apply_filters('taiowc_print_notices_html', $notices_html, $taiowc_notices, $section ));
        
        $this->taiowc_notices = array();

       }


       public function taiowc_HideCartPage() {

        if (isset($this->taiowc_HideCartPage)) {
            return $this->taiowc_HideCartPage;
        }
    
        if (!trim(taiowc_main()->taiowc_get_option('taiowc-not_showing_page'))) {
            $hidePages = array();
        } else {
            $hidePages = array_map('trim', explode(',', taiowc_main()->taiowc_get_option('taiowc-not_showing_page')));
        }
        
        if(is_shop()){
            $shopPageID = get_option('woocommerce_shop_page_id');
            $isShopPageHidden = false;
    
            if ($shopPageID) {
            $shopPage = get_post($shopPageID); 
            $isShopPageHidden = ($shopPage && in_array($shopPage->post_name, $hidePages));
            } 
        }else{
            $isShopPageHidden = false;  
        }
       
    
        $this->isSideCartPage = !( !empty($hidePages) && (
            (in_array('no-woocommerce', $hidePages) && !is_woocommerce() && !is_cart() && !is_checkout()) ||
            is_page($hidePages) ||
            (is_product() && in_array(get_the_id(), $hidePages)) || $isShopPageHidden
        ) );
    
        foreach ($hidePages as $page_id) {
            if (is_single($page_id)) {
                $this->isSideCartPage = false;
                break;
            }
        }
    
        return apply_filters('taiowc_is_sidecart_page', $this->isSideCartPage, $hidePages);
    }
    
  }

// Load Plugin

function taiowc_main(){

return Taiowc_Main::instance();

}

add_action( 'init', 'taiowc_main', 25 );

endif; 
